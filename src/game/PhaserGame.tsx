...
import React, {forwardRef, useEffect, useLayoutEffect, useRef, useState} from 'react'; // Added useState
...
export const PhaserGame = forwardRef<PhaserGameRef, PhaserGameProps>(function PhaserGame({}, ref) {
    ...
    useLayoutEffect(() => {
        if (gameRef.current === null && containerRef.current) {
            ...
            gameRef.current.events.on('scene-ready', (sceneInstance: Phaser.Scene) => {
                if (sceneInstance.scene.key === 'Game') {
                    console.log('PhaserGame: Game scene is ready, setting up bridge listeners.');
                    setGameScene(sceneInstance); // Store the scene instance
                }
            });
            // Also listen for other scenes if needed, e.g., MainMenu readiness
            gameRef.current.events.on('scene-ready', (sceneInstance: Phaser.Scene) => {
                if (sceneInstance.scene.key === 'MainMenu' || sceneInstance.scene.key === 'GameOver') {
    ...
    useEffect(() => {
        if (gameScene) {
            console.log("PhaserGame: Attaching listeners to Game scene's internal events.");
            ...
            return () => {
                console.log("PhaserGame: Removing listeners from Game scene's internal events.");
    ...
    return <div id="game-container" ref={containerRef} style={{width: '100%', height: '100%'}}/>;
